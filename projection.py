...
class ProjectionHelper:
    ...
    def compute_proj_idcs(self, cam2world, grid2world):
        ...
        lin_ind_frustrum = torch.arange(0, num_frust_elements).long()
        ...
        tmp = lin_ind_frustrum - (
                    coords[2] * self.projection_image_dims[0] * self.projection_image_dims[1]).long()
...
def interpolate_lifting(image, lin_ind_3d, query_points, grid_dims):
    ...
    output = torch.zeros(1, num_feats, grid_dims[0] * grid_dims[1] * grid_dims[2])
...
def interpolate_trilinear(grid, lin_ind_frustrum, voxel_coords, img_shape, frustrum_depth):
    ...
    output = torch.zeros(batch, num_feats, img_shape[0] * img_shape[1] * frustrum_depth)
