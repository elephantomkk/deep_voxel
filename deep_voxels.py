...
class DeepVoxels(nn.Module):
    ...
    def forward(self,
        ...
        if input_img is not None:
            ...
            # img_var = Variable(input_img) # assign it to a variable
            # print(img_var.shape)
            # features_var = resnet18(img_var) # get the output from the last hidden layer of the pretrained resnet
            # img_feats = features_var.data # get the tensor out of the variable
            ...
            outputs = res50_conv2(input_img)
            # img_feats = res50_conv2(input_img)
            # print(outputs.shape)
            # print(outputs.data.shape) # => torch.Size([4, 2048, 7, 7]) [1, 256, 56, 56])

            img_feats = self.feature_extractor(outputs)
